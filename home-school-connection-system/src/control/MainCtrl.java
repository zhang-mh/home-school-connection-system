	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("login"))
		{
			...
			if(!pagerandom.equals(random)&&request.getParameter("a")!=null)
			{
				...
				String sql1="";
	if (utype.equals("教师")){sql1 = "select * from jiaoshixinxi where jiaoshihao='"+username+"' and mima='"+password+"' ";}
	if (utype.equals("家长")){sql1 = "select * from jiachangxinxi where jiachangzhanghao='"+username+"' and mima='"+password+"' and issh='是'";}
			List<HashMap> userlist1 = dao.select(sql1);
			if(userlist1.size()==1)
			{
				if (utype.equals("教师"))
				{
					request.getSession(). setAttribute("username", userlist1.get(0).get("jiaoshihao"));  
				}
				else
				{
					request.getSession(). setAttribute("username", userlist1.get(0).get("jiachangzhanghao"));  
				}
